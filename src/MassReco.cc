...
pair<double, double> MassReco::doReco(vlq::JetCollection ak4Jets, vlq::Jet fatJet, double bosMass, TLorentzVector Leptons, double pT){
  ...
  for (int mass=0; mass<=2000; mass+=5){
	...
	ak4[2].SetPtEtaPhiM(0.,0.,0.,0.);
     ...
     while(std::next_permutation(index_array, index_array + 3));
...
pair<double, double> MassReco::doReco(vlq::JetCollection collection, double bosMass, TLorentzVector Leptons, double pT){
   // int next = 0;
   //  for(int i=0; i<5; i++){
   ...
   //  }
    ...
    for (int mass = 0; mass <= 2000; mass+=5){
		...
		Jets[4] = collection.at(i4).getP4();
                ...
                loop = chi2(passToChi2, Leptons, bosMass,  mass, pT);
...
double MassReco::chi2(vector<TLorentzVector> jets, TLorentzVector Leptons, double bosMass, double mass, double pT){

  if ((jets[2] + jets[3]).Pt() > pT){
	...
	double term2 = BHup2 / (98.435*98.435);

	double BLup = abs((jets[0] + Leptons).M() - mass);
	double BLup2 = BLup * BLup;
	double term3 = BLup2 / (64.68*64.68);
